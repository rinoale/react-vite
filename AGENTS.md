...
All OCR scripts are under `scripts/ocr/`, organized by model type. All accept `--version <ver>` (defaults to active version from symlink).
1. **Generate Data:** `python3 scripts/ocr/general_model/generate_training_data.py` — Creates synthetic binary training images using the Mabinogi game font. Images must be strictly binary (0/255 only), matching frontend preprocessing.
2. **Create LMDB:** `python3 skills/ocr-trainer/scripts/create_lmdb_dataset.py --input backend/ocr/general_model/<ver>/train_data --output backend/ocr/general_model/<ver>/train_data_lmdb`
3. **Train:** `python3 scripts/ocr/general_model/train.py` — reads version's `training_config.yaml` and runs `deep-text-recognition-benchmark/train.py`. See `OCR_TRAINING_HISTORY.md` for details and past issues.
4. **Deploy:** `bash scripts/ocr/general_model/deploy.sh <version>` — copies to version folder + updates symlinks.

### Custom Model Configuration
- Requires `backend/ocr/models/custom_mabinogi.yaml` generated by `scripts/ocr/general_model/create_model_config.py`.
- Requires `backend/ocr/models/custom_mabinogi.py` (copied from training repo) to define the architecture for EasyOCR.
