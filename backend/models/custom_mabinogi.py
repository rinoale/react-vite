...
class Model(nn.Module):
    ...
    def __init__(self, opt=None, **kwargs):
        ...
        # Helper class to act as a Namespace if opt is missing
        class Opt: pass
        
        if opt is None:
            opt = Opt()
            # Set defaults or populate from kwargs
            ...
            opt.num_class = int(kwargs.get('num_class', 1))
            opt.batch_max_length = int(kwargs.get('batch_max_length', 25))
        else:
            # Update opt with any kwargs overrides
