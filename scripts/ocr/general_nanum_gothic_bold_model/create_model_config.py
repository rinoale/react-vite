#!/usr/bin/env python3
"""
Generate custom_nanum_gothic_bold.yaml for EasyOCR inference.

Reads model architecture params from the version's training_config.yaml
and character list from unique_chars.txt.

Run from project root:
    python3 scripts/ocr/general_nanum_gothic_bold_model/create_model_config.py
    python3 scripts/ocr/general_nanum_gothic_bold_model/create_model_config.py --version a19
"""
import argparse
import os
import sys
...
parser = argparse.ArgumentParser(description='Generate EasyOCR inference yaml for nanum_gothic_bold model')
...
print(f"  bash scripts/ocr/general_nanum_gothic_bold_model/deploy.sh {version}")
