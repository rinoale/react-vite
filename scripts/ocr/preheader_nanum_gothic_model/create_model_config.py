#!/usr/bin/env python3
"""
Generate custom_preheader_nanum_gothic.yaml for EasyOCR inference.

Reads model architecture params from the version's training_config.yaml
and character list from unique_chars.txt.

Run from project root:
    python3 scripts/ocr/preheader_nanum_gothic_model/create_model_config.py
    python3 scripts/ocr/preheader_nanum_gothic_model/create_model_config.py --version v1
"""
import argparse
import os
import sys
...
parser = argparse.ArgumentParser(description='Generate EasyOCR inference yaml for preheader_nanum_gothic model')
...
print(f"  bash scripts/ocr/preheader_nanum_gothic_model/deploy.sh {version}")
