#!/usr/bin/env python3
"""
Training launcher for pre-header NanumGothicBold font OCR model.

Reads training_config.yaml from the version folder and runs train.py.

Usage (from project root):
    python3 scripts/ocr/preheader_nanum_gothic_model/train.py
    python3 scripts/ocr/preheader_nanum_gothic_model/train.py --version v1
    python3 scripts/ocr/preheader_nanum_gothic_model/train.py --resume
    python3 scripts/ocr/preheader_nanum_gothic_model/train.py --num_iter 20000
"""
import argparse
import os
import subprocess
...
def build_command(cfg, overrides):
    ...
    exp_name_prefix = training.get("exp_name", "preheader_nanum_gothic_ocr")
...
def main():
    parser = argparse.ArgumentParser(description="Train preheader_nanum_gothic OCR model")
    ...
    print("=" * 60)
    print("  OCR Training Launcher — preheader_nanum_gothic")
    print("=" * 60)
