...
export default function SouthKoreaMap() {
  ...
  useEffect(() => {
    ...
    script.onerror = () => {
      ...
      setSearchError(t('navigate.failedMapLoad'));
    };
  ...
  useEffect(() => {
    ...
    const cities = [
      { name: t('navigate.cities.seoul'), lat: 37.5665, lng: 126.9780 },
      { name: t('navigate.cities.busan'), lat: 35.1796, lng: 129.0756 },
      { name: t('navigate.cities.incheon'), lat: 37.4563, lng: 126.7052 },
      { name: t('navigate.cities.daegu'), lat: 35.8714, lng: 128.6014 },
      { name: t('navigate.cities.daejeon'), lat: 36.3504, lng: 127.3845 },
      { name: t('navigate.cities.gwangju'), lat: 35.1595, lng: 126.8526 },
      { name: t('navigate.cities.ulsan'), lat: 35.5384, lng: 129.3114 },
      { name: t('navigate.cities.jejuCity'), lat: 33.4996, lng: 126.5312 },
    ];
  ...
  useEffect(() => {
    ...
    userMarkerRef.current = window.L.marker([userLocation.lat, userLocation.lng], { icon: userIcon })
      ...
      .bindPopup(`<b>${t('navigate.yourLocation')}</b>`);
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (data.length > 0) {
        ...
        setSearchError(t('navigate.locationNotFound'));
      }
    } catch (error) {
      setSearchError(t('navigate.errorSearching'));
  ...
  const requestUserLocation = async (silent = false) => {
    if (!('geolocation' in navigator)) {
      if (!silent) {
        setLocationError(t('navigate.geoNotSupported'));
      }
      return;
    }
    ...
    try {
      ...
      let errorMessage = t('navigate.unableToGetLocation');

      switch (error.code) {
        case error.PERMISSION_DENIED:
          errorMessage = t('navigate.locationDenied');
          break;
        case error.POSITION_UNAVAILABLE:
          errorMessage = t('navigate.locationUnavailable');
          break;
        case error.TIMEOUT:
          errorMessage = t('navigate.locationTimeout');
          break;
      }
  ...
  return (
    <div className="w-full h-screen flex flex-col">
      <div className="bg-blue-600 text-white p-4 shadow-lg">
        <h1 className="text-2xl font-bold">{t('navigate.title')}</h1>
        <p className="text-sm mt-1">{t('navigate.subtitle')}</p>
        ...
        <form onSubmit={handleSearch} className="mt-4 flex gap-2">
          ...
          >
            <Search size={20} />
            {searching ? t('navigate.searching') : t('navigate.search')}
          </button>
        </form>
        ...
        <div className="mt-3 flex gap-2">
          ...
          >
            ...
            {isGettingLocation ? t('navigate.gettingLocation') : t('navigate.getMyLocation')}
          </button>
      ...
      <div className="relative flex-1">
        ...
        {userLocation && (
          <div className="absolute top-4 left-4 bg-white text-gray-800 px-3 py-2 rounded-lg shadow-lg text-sm flex items-center gap-2 z-10">
            ...
            <span>{t('navigate.locationActive')}</span>
          </div>
        )}
        ...
        {!userLocation && mapReady && (
          <div className="absolute bottom-4 left-4 bg-yellow-100 border border-yellow-300 text-yellow-800 px-3 py-2 rounded-lg shadow-lg text-sm max-w-xs z-10">
            <p className="font-medium mb-1">{t('navigate.enableLocation')}</p>
            <p className="text-xs">{t('navigate.enableLocationHint')}</p>
          </div>
        )}
      </div>
      ...
      {!mapReady && (
        <div className="absolute inset-0 flex items-center justify-center bg-gray-100">
          <p className="text-gray-600">{t('navigate.loadingMap')}</p>
        </div>
      )}
    </div>
  );
