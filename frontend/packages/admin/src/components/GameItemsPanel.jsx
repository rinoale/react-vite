...
const GameItemsPanel = () => {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const { data } = await getGameItems({ limit: pagination.limit, offset: pagination.offset });
      setRows(data.rows || []);
    } catch (error) {
  ...
  return (
    <div className={panelOuter}>
      <div className={panelHeader}>
        <h2 className={panelTitle}>{t('nav.items.game_items')}</h2>
      ...
      {isLoading && rows.length === 0 ? (
        ...
        <div className={panelEmpty}>No game items found</div>
      ) : (
